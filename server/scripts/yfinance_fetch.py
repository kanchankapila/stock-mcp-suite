...
Usage:
  - Single symbol JSON (back-compat):
      python server/scripts/yfinance_fetch.py <SYMBOL> [--period 6mo] [--interval 1d]
  ...
  - Ingest all symbols from stocklist.ts into SQLite (stock.db):
      python server/scripts/yfinance_fetch.py --all [--period 6mo] [--interval 1d]
    Reads symbols from server/stocklist.ts, appends Yahoo suffix (default .NS),
    fetches daily history via yfinance, and upserts into prices table of stock.db.
"""
...
def ensure_deps() -> None:
    try:
        ...
        # Attempt to install when missing
        try:
            cmd = f"{shlex.quote(sys.executable)} -m pip install --quiet yfinance pandas"
            subprocess.check_call(cmd, shell=True)
        except Exception as e:  # pragma: no cover
            fail(
                "Missing dependencies. Install with: pip install yfinance pandas\n"
...
def parse_args(argv: List[str]) -> Dict[str, str]:
    # Defaults
    out: Dict[str, str] = {"symbol": "", "period": "6mo", "interval": "1d", "all": "false"}
    if len(argv) < 2:
...
def main(argv: List[str]) -> None:
    ...
    if do_all:
        # Batch mode: read symbols, append Yahoo suffix and ingest to SQLite
        ...
        for base, name in entries:
            ...
            try:
                ...
                # Upsert extra Yahoo data as JSON caches
                ...
                # Keep major holders summary as JSON for now
                try:
                    mh = df_records(major)
                ...
                total_symbols += 1
                total_rows += n
                print(json.dumps({"ok": True, "symbol": ysym, "inserted": n}))
            except Exception as e:
                msg = f"{ysym}: {e}"
                errors.append(msg)
                print(json.dumps({"ok": False, "symbol": ysym, "error": str(e)}), file=sys.stderr)
        # Summary
        summary = {"ok": True, "mode": "all", "symbols": total_symbols, "rows": total_rows, "errors": len(errors), "db": str(db_path)}
        print(json.dumps(summary, ensure_ascii=False))
        return
    else:
        # Single symbol JSON output mode (back-compat)
        try:
            t = yf.Ticker(sym)
